...
class Quote(db.Model):
    ...
    # Add this line to establish the relationship with Booking
    bookings = db.relationship('Booking', back_populates='quote')
...
class Booking(db.Model):
    ...
    residence_type = db.relationship('Residence', back_populates='bookings')

class Residence(db.Model):
    serialize_rules = ('-inventory.residence_type', '-quote.residence_type', '-booking.residence_type')
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(50), unique=True, nullable=False)
    bookings = db.relationship('Booking', back_populates='residence_type')
